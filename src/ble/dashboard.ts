  ...
  <main>
    ...
    <section class="wide"><div class="section-head"><h2>Circadian service</h2><span id="circadian-generated">Loading schedule</span></div>
      ...
      <div class="graph-wrap">
        <svg id="circadian-graph" viewBox="0 0 1000 300" role="img" aria-label="Circadian Kelvin and intensity schedule"></svg>
        <div id="circadian-tooltip" class="graph-tooltip" hidden></div>
      </div>
      <div class="graph-legend"><span class="kelvin-key">Kelvin</span><span class="intensity-key">Natural intensity</span><span>Hover or slide over the graph for exact values</span></div>
    </section>
...
function renderCircadian(){if(!circadian)return;const service=circadian.service,current=circadian.current,schedule=circadian.schedule;$('circadian-generated').textContent='Updated '+new Date(circadian.generatedAt).toLocaleTimeString();$('circadian-active').textContent=service.healthy?'Active':service.active?'Loaded, waiting':'Inactive';statusClass($('circadian-active'),service.healthy?'good':service.active?'warn':'bad');$('circadian-last-run').textContent=service.lastRunAt?'Last run '+new Date(service.lastRunAt).toLocaleTimeString():'No recorded run';const target=service.lastTarget||current;$('circadian-target').textContent=target?Math.round(target.cct)+' K · '+target.intensity+'%':'—';const applied=status?.lighting?Object.values(status.lighting)[0]:undefined;$('circadian-applied').textContent=applied?.cct!==undefined?'Applied '+applied.cct+' K · '+applied.intensity/10+'%':'No fixture readback';$('circadian-curve').textContent=(current?.curve||service.curve||'Unknown').replaceAll('-',' ');if(schedule?.points?.length){const intensities=schedule.points.map(point=>point.intensity),ccts=schedule.points.map(point=>point.cct);$('circadian-bounds').textContent=Math.min(...ccts)+'–'+Math.max(...ccts)+' K · '+Math.min(...intensities)+'–'+Math.max(...intensities)+'%'}else $('circadian-bounds').textContent=circadian.calculationError||'Schedule unavailable';$('circadian-weather').textContent=service.weatherConfigured?(current?.weatherActive?'Active':'Configured, unavailable'):'Off';statusClass($('circadian-weather'),current?.weatherActive?'good':service.weatherConfigured?'warn':'');const weather=[];if(current?.cloudCover!==undefined)weather.push(Math.round(current.cloudCover*100)+'% cloud');if(current?.precipitation)weather.push(current.precipitation);if(current?.weatherDataSource)weather.push(current.weatherDataSource);$('circadian-weather-detail').textContent=weather.join(' · ')||'No weather adjustment';renderCircadianGraph(schedule)}
function renderCircadianGraph(schedule){const svg=$('circadian-graph'),tooltip=$('circadian-tooltip');if(!schedule?.points?.length){svg.innerHTML='<text x="500" y="150" text-anchor="middle" fill="#9ca9a1">Schedule unavailable</text>';return}const points=schedule.points,width=1000,height=300,left=64,right=34,top=22,bottom=42,plotWidth=width-left-right,plotHeight=height-top-bottom,minCct=Math.min(...points.map(point=>point.cct)),maxCct=Math.max(...points.map(point=>point.cct)),cctSpan=Math.max(1,maxCct-minCct),intensityLimit=Math.max(0,Math.min(100,schedule.intensityLimit??100));const x=index=>left+index/(points.length-1)*plotWidth,yCct=value=>top+(maxCct-value)/cctSpan*plotHeight,yIntensity=value=>top+(100-value)/100*plotHeight,line=values=>values.map((value,index)=>x(index).toFixed(1)+','+value.toFixed(1)).join(' '),kelvin=line(points.map(point=>yCct(point.cct))),intensity=line(points.map(point=>yIntensity(point.intensity))),limitY=yIntensity(intensityLimit),now=Date.now(),currentIndex=points.reduce((best,point,index)=>Math.abs(Date.parse(point.time)-now)<Math.abs(Date.parse(points[best].time)-now)?index:best,0),grid=[0,.25,.5,.75,1].map(factor=>{const y=top+factor*plotHeight;return '<line x1="'+left+'" y1="'+y+'" x2="'+(width-right)+'" y2="'+y+'" stroke="#26302b"/>'}).join(''),labels=[0,6,12,18,24].map(hour=>{const index=Math.min(points.length-1,Math.round(hour/24*(points.length-1)));return '<text x="'+x(index)+'" y="'+(height-14)+'" text-anchor="middle" fill="#9ca9a1" font-size="12">'+String(hour).padStart(2,'0')+':00</text>'}).join('');svg.innerHTML=grid+'<text x="10" y="'+(top+5)+'" fill="#79f2a7" font-size="12">'+maxCct+' K</text><text x="10" y="'+(top+plotHeight)+'" fill="#79f2a7" font-size="12">'+minCct+' K</text><text x="'+(width-5)+'" y="'+(top+5)+'" text-anchor="end" fill="#ffbb55" font-size="12">100%</text><text x="'+(width-5)+'" y="'+(top+plotHeight)+'" text-anchor="end" fill="#ffbb55" font-size="12">0%</text>'+labels+'<line x1="'+left+'" y1="'+limitY+'" x2="'+(width-right)+'" y2="'+limitY+'" stroke="#ffbb55" stroke-width="2" stroke-dasharray="6 6" opacity=".75"/><text x="'+(width-right-4)+'" y="'+(limitY-6)+'" text-anchor="end" fill="#ffbb55" font-size="12">'+intensityLimit+'% service limit</text><polyline points="'+kelvin+'" fill="none" stroke="#79f2a7" stroke-width="3"/><polyline points="'+intensity+'" fill="none" stroke="#ffbb55" stroke-width="3"/><line x1="'+x(currentIndex)+'" y1="'+top+'" x2="'+x(currentIndex)+'" y2="'+(top+plotHeight)+'" stroke="#eef5f0" stroke-dasharray="5 5" opacity=".65"/><line id="circadian-crosshair" x1="0" y1="'+top+'" x2="0" y2="'+(top+plotHeight)+'" stroke="#eef5f0" opacity="0"/><circle id="circadian-k-dot" r="5" fill="#79f2a7" opacity="0"/><circle id="circadian-i-dot" r="5" fill="#ffbb55" opacity="0"/>';const show=event=>{const box=svg.getBoundingClientRect(),ratio=Math.max(0,Math.min(1,(event.clientX-box.left)/box.width)),index=Math.round(ratio*(points.length-1)),point=points[index],px=x(index),cross=$('circadian-crosshair'),kdot=$('circadian-k-dot'),idot=$('circadian-i-dot');cross.setAttribute('x1',px);cross.setAttribute('x2',px);cross.setAttribute('opacity','1');kdot.setAttribute('cx',px);kdot.setAttribute('cy',yCct(point.cct));kdot.setAttribute('opacity','1');idot.setAttribute('cx',px);idot.setAttribute('cy',yIntensity(point.intensity));idot.setAttribute('opacity','1');tooltip.hidden=false;tooltip.innerHTML='<strong>'+new Date(point.time).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'})+'</strong><br>'+point.cct+' K<br>Natural '+point.intensity+'%<br>Service applies '+point.appliedIntensity+'%'+(point.lightOutput!==undefined?'<br>'+Math.round(point.lightOutput).toLocaleString()+' schedule lux':'');tooltip.style.left=Math.min(box.width-190,Math.max(8,event.clientX-box.left+12))+'px';tooltip.style.top=Math.max(8,event.clientY-box.top-94)+'px'};svg.onpointermove=show;svg.onpointerdown=show;svg.onpointerleave=()=>{tooltip.hidden=true;for(const id of ['circadian-crosshair','circadian-k-dot','circadian-i-dot'])$(id)?.setAttribute('opacity','0')}}
