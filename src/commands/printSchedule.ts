...
function _displayAllSpecialTimes(times: Record<string, Date | undefined>): void {
...
function registerPrintSchedule(program: Command, deps: CommandDeps) {
  ...
  program
    ...
    .action(
      asyncCommand(async (options: CommandOptions & ScheduleCommandOptions & { csv?: boolean; output?: string }) => {
        ...
        if (options.output) {
          try {
            ...
            // Only log success message to console if we are not suppressing output?
            // Actually console.log is fine here, since we are outputting to file, the user sees nothing unless we tell them.
