...
export function registerColor(program: Command, deps: CommandDeps) {
  ...
  program
    ...
    .description('Set color by name or hex code (e.g., "red", "#ff0000"). Omit device or use "all" to set all lights.')
    .option('-i, --intensity <value>', 'Set intensity (0-100)')
    .option('-u, --url <url>', 'WebSocket URL')
    .option('-c, --client-id <id>', 'Client ID')
    .option('-d, --debug', 'Enable debug mode')
    .action(
      asyncCommand(async (color: string, deviceQuery: string | undefined, options: CommandOptions) => {
        ...
        if (!deviceQuery || deviceQuery.toLowerCase() === 'all') {
          await controller.setColorForAllLights(color, intensity, (success: boolean, message: string) => {
            if (!success) {
        ...
        controller.setColor(device.node_id, color, intensity, (success: boolean, message: string) => {
          if (success) {
