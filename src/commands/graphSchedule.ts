...
export function registerGraphSchedule(program: Command, deps: CommandDeps) {
  ...
  program
    ...
    .action(
      asyncCommand(async (options: CommandOptions & GraphCommandOptions) => {
        ...
        // --- 1. Determine Location & Date (Copied/Adapted from printSchedule) ---
        ...
        // Determine Lat/Lon (logic remains same)
        ...
        // --- 3. Generate Data Points (Daylight Hours + Buffer) ---
        ...
        // Determine efficient range: nightEnd (or sunrise - buffer) to night (or sunset + buffer)
        // If nightEnd/night are invalid (polar day/night), fall back to 00:00-23:59 or reasonable defaults
        if (nightEnd && !Number.isNaN(nightEnd.getTime()) && night && !Number.isNaN(night.getTime())) {
          ...
          // Fallback to sunrise/sunset with larger buffer if night times missing
        ...
        // Add buffer to show flatlines (e.g. 60 mins before/after)
        ...
        // Generate points every minute within range
        ...
        for (let i = 0; i <= totalMinutes; i++) {
          ...
          // Label logic: Show every hour on the hour
        ...
        curveTypes.forEach((curve, index) => {
          ...
          if (showIntensity) {
            datasets.push({
              ...
              yAxisID: 'y',
              fill: false,
              borderDash: showIntensity && showCct ? [5, 5] : [], // Dotted line only if mixing Intensity & CCT
              pointRadius: 0,
              borderWidth: 2,
            });
          }
        ...
        // Also get sun times for annotation lines (re-using calculated times)
        const _sunEvents = [
          { name: 'Sunrise', time: times.sunrise, color: 'rgba(255, 205, 86, 0.8)' },
          { name: 'Solar Noon', time: times.solarNoon, color: 'rgba(255, 159, 64, 0.8)' },
          { name: 'Sunset', time: times.sunset, color: 'rgba(75, 192, 192, 0.8)' },
        ].filter((e) => e.time && !Number.isNaN(e.time.getTime()));
        ...
        const configuration: any = {
          ...
          options: {
            plugins: {
              ...
              annotation: {
                // Chart.js annotation plugin might not be installed, so we skimp on this for now or draw vertically manually if we want complex stuff.
                // Actually, chartjs-node-canvas doesn't include plugins by default.
              },
            },
            scales: {
              x: {
                ticks: {
                  ...
                  callback: (_val: any, index: number) => {
                    // Labels are already empty string except for round hours
                    return labels[index];
                  },
                },
              },
